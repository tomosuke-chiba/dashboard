...
| 機能 | 詳細 | ステータス |
|------|------|------------|
| 職種別データ取得 | Dr / DH / DA のタブ/フィルター切り替えで取得 | 未実装 |
| スカウトメール送信数 | `/service/message_thread?filter_tab=scout_no_reply` から取得 | 未実装 |
| スカウトメール返信数 | `/service/message_thread` から取得 | 未実装 |
| 閲覧率30%超アラート | Discord通知で警告 | 未実装 |
| Bitly連携 | API連携でクリック数自動取得 | 未実装 |
