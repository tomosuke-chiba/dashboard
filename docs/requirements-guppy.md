...
| 項目 | ソースURL | 取得方法 | 実装状況 |
|------|-----------|----------|----------|
| 送信数 | `/service/message_thread?filter_tab=scout_no_reply` + `type_scout`フィルタ | 全スカウトスレッドからスカウトメモを抽出してカウント | ✅完了 |
| 返信数 | 差分計算 | 全スカウトスレッド - 未返信スレッド = 返信ありスレッド | ✅完了 |
...
- `scout_no_reply`タブから未返信スレッドIDを取得
- `type_scout`フィルタでスカウト全体を取得
- 差分で返信ありスレッドを特定し、日別に返信数を集計
...
**実装詳細:**
- `scrapeGuppyScoutMessages`関数でスカウトメモから送信数・返信数を取得
- スカウトメモ形式: "1226 DH スカウト" → 日付と職種を抽出
- `scout_no_reply`タブから未返信スレッドIDを取得
- `type_scout`フィルタでスカウト全体を取得し、差分で返信ありを特定
- 日別送信数・返信数をDBに保存
